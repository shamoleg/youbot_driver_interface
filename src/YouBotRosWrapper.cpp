...
YouBotBaseWrapper::YouBotBaseWrapper(NodeHandle n):
node(n)
{
    subSetpointPosition = node.subscribe("base/joint_state", 1, &YouBotBaseWrapper::callbackSetpointPosition, true)
    subSetpointVelocity = node.subscribe("base/joint_state", 1, &YouBotBaseWrapper::callbackSetpointVelocity, true)
    subSetpointJoint = node.subscribe("base/joint_state", 1, &YouBotBaseWrapper::callbackSetpointJointState, true)
}

YouBotBaseWrapper::callbackSetpointPosition()
{
    
}

YouBotBaseWrapper::callbackSetpointVelocity()
{

}

YouBotBaseWrapper::callbackSetpointJointState()
{
